...
if __name__ == '__main__':
    ...
    for n, time_shift in enumerate([30,60,90]) :
        for fname in ['anomaly___', ''] :

            path = f'output/{args.dataset}/'
            if fname=='' : path = f'../organization/output/merged/{args.dataset}/'

            df   = pd.read_csv(f'{path}P9/{fname}P9_base.csv')
            ...
            df1.to_csv(f'{folder_out}{fname}F2_base.csv',           index=False)
            df2.to_csv(f'{folder_out}{fname}F2_{time_shift}min_later.csv',    index=False)
